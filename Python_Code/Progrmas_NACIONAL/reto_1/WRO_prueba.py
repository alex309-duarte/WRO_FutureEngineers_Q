...
def initialize_Libraries():
    ...
    spike.readline() #clear buffer
    spike.write("print(motion_sensor.tilt_angles())\r".encode())   
    spike.readline() #clear buffer
    spike.write("return 255\r".encode())
    spike.readline() #clear buffer
    end_Function()
...
def avanzar_detection_izquierdo_lidar(vel,referencia):
    ...
    try:
        print("avanzando con lidar")
        while not vision_queue.full():
            print("queue full waiting avanzar\n")
        obj = vision_queue.get()
        while obj.dist_left > 0 and obj.dist_left < 1350:
            spike.write(("da("+str(vel)+","+str(referencia)+")\r").encode())
            spike.readline() #clear buffer
            if vision_queue.full():
                obj = vision_queue.get()
            print("avanzando",obj.dist_left)
            time.sleep(0.001)
        Coast_motors()
    except queue.Empty:
        pass
...
def RPlidar_worker():
    try:
        lidar.clean_input()
        angles = [0]*360
        for scan in lidar.iter_scans( min_len=0):
