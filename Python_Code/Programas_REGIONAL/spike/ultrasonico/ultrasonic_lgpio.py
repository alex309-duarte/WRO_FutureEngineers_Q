...
SENSORS = [
    ...
    UltrasonicSensor("derecha", trigger_pin=26, echo_pin=26)
]

# Performance tuning parameters
MIN_MEASURE_INTERVAL = 0.03  # 30ms between measurements (about 33Hz per sensor)
SENSOR_TIMEOUT = 0.05       # 50ms timeout for echo pulse (reduced from 100ms)
...
def main():
    ...
    try:
        ...
        while running:
            try: 
                uls = [1,1,1]             
                uls = avanzar_detection(90,0)
                print(uls)
                if uls[0] == -1:
                    vuelta_automatica(70,90,uls)
                    centrar_vehiculo()
                    o = 0
                    while o < 11:
                        avanzar_recto_grados(70,720,90)
                        seguir_pared_izquierdo(70,1700)
                        reset_gyro(0)
                        uls = avanzar_detection(90,0)
                        avanzar_recto_grados(70,50,0)
                        vuelta_automatica(70,90,uls)
                        centrar_vehiculo()
                        o = o + 1
                ...
                if uls[2] == -1:
                    ...
                    while o < 11:
                        ...
                        centrar_vehiculo()
                        o = o + 1
                seguir_pared_derecho(70,1200)
                Coast_motors()
                running = False
                print("movimiento acabado\n")
                # Display readings at specified rate
                # Process queued updates with timeout
                #avanzar_detection(60,0)
                #vuelta_automatica(60,90)
                #avanzar_distancia(60,150,0)
                #seguir_pared_izquierdo(60,7200)
                # Small sleep to prevent 100% CPU usage, but shorter than before
